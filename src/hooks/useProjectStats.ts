  ...
  useEffect(() => {
    ...
    const fetchProjectStats = async () => {
      try {
        ...
        // Получаем кристаллы проекта из таблицы projects
        ...
        if (projectError) {
          console.error('Error fetching project crystals:', projectError)
          setError('Failed to load project crystals')
          return
        }
        ...
        if (ticketStats && ticketStats.length > 0) {
          ...
          setStats(prevStats => ({
            ...prevStats,
            diamonds: projectData?.crystals || 0, // Используем кристаллы из таблицы projects
          ...
          // Если нет статистики тикетов, все равно показываем кристаллы проекта
          setStats(prevStats => ({
            ...prevStats,
            diamonds: projectData?.crystals || 0
          }))
        }
